                ...
                <div class="row">
                        ...
                        <div class="row">
                            ...
                            <?php
                                ...
                                if(isset($_REQUEST['filter']))
                                {
                                    ...
                                    $stype=mysqli_real_escape_string($con, trim($_REQUEST['stype']));
                                    $city=mysqli_real_escape_string($con, trim($_REQUEST['city']));
                                    
                                    $conditions = ["property.uid=user.uid"];
                                    if(!empty($type))  $conditions[] = "LOWER(type)=LOWER('$type')";
                                    if(!empty($stype)) $conditions[] = "LOWER(stype)=LOWER('$stype')";
                                    if(!empty($city))  $conditions[] = "LOWER(city) LIKE LOWER('%$city%')";
                                    
                                    $sql="SELECT property.*, user.uname FROM `property`,`user` WHERE " . implode(" AND ", $conditions);
                            ...
                            <div class="col-md-6">
                                <div class="featured-thumb hover-zoomer mb-4">
                                    <div class="overlay-black overflow-hidden position-relative"> <img src="admin/property/<?php echo $row['18'];?>" alt="pimage">
                                        ...
                                        <div class="price text-primary text-capitalize">TND<?php echo $row['13'];?> <span class="text-white"><?php echo $row['12'];?> Sqft</span></div>
                    ...
                    <div class="col-lg-4">
                        ...
                        <form class="d-inline-block w-100" action="calc.php" method="post">
                            <input type="hidden" name="calctype" value="sale" id="calcTypeInput">
                            <label class="sr-only">Property Amount</label>
                            <div class="input-group mb-2 mr-sm-2">
                                <div class="input-group-prepend">
                                    <div class="input-group-text">TND</div>
                                </div>
                                <input type="text" class="form-control" name="amount" placeholder="Monthly Rent / Property Price">
                            </div>
                            ...
                            <div id="interestGroup" class="input-group mb-2 mr-sm-2">
                                <div class="input-group-prepend">
...
<script>
// Auto-detect if page has rent properties and switch calculator mode
(function(){
    var saleTag = document.querySelector('.sale');
    if(saleTag){
        var txt = saleTag.innerText.toLowerCase();
        if(txt.indexOf('rent') !== -1){
            var ci = document.getElementById('calcTypeInput');
            var ig = document.getElementById('interestGroup');
            if(ci) ci.value = 'rent';
            if(ig) ig.style.display = 'none';
            var h4 = document.querySelector('.double-down-line-left');
            if(h4) h4.innerText = 'Rental Cost Calculator';
        }
    }
})();
</script>
